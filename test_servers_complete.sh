...
echo "步骤5：测试连接测试..."
csrf_token=$(grep 'csrf_token' /tmp/cookies.txt | awk '{print $7}')
connect_response=$(curl -s -X POST http://localhost:8080/api/v1/servers/$server_id/test-connection \
  -H "Authorization: Bearer $access_token" \
  -H "X-CSRF-Token: $csrf_token" \
  -b /tmp/cookies.txt)
...
if echo "$connect_response" | grep -q "success\|connected"; then
    ...
    echo "⚠️ 连接测试失败（可能是ProxySQL ACL配置问题）"
fi
echo ""
...
echo "步骤6：测试删除服务器配置..."
csrf_token=$(grep 'csrf_token' /tmp/cookies.txt | awk '{print $7}')
delete_response=$(curl -s -X DELETE http://localhost:8080/api/v1/servers/$server_id \
  -H "Authorization: Bearer $access_token" \
  -H "X-CSRF-Token: $csrf_token" \
  -b /tmp/cookies.txt)
